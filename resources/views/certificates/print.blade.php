...
<div id="borde">
    ...
    <table width="100%">
        <tr>
            <td style="text-align: center;  width:70%">
                <h1 style="margin-bottom: 0px; margin-top: 5px">
                    {{str_pad($certificate->code->code, 7, "0", STR_PAD_LEFT)}}-{{$certificate->code->initials}}
                </h1>
            </td>
        </tr>
    </table>
    <table width="100%">
        <tr style="text-transform: uppercase;">
            <td style="text-align: center;  width:70%">
                <h1 style="margin-bottom: 0px; margin-top: 5px">
                    {{$certificate->company->miningOperator}}
                </h1>
            </td>
        </tr>
    </table>
    ...
    <div class="text" style="text-transform: uppercase;">
        <p style="font-size: 16px;">
            ...
            <b>MUNICIPIO: {{$certificate->company->municipe}}</b>
        </p>
    </div>
    <table width="100%">
        <tr>
    ...
    @endphp
    <div class="text">
        <p style="font-size: 13px;"><b>VALIDO HASTA:</b>  {{$fecha1->format('d')}} de {{$mes1}} de {{$fecha1->format('Y')}}<br><b>FECHA DE EMISION:</b> {{$fecha->format('d')}} de {{$mes}} de {{$fecha->format('Y')}}</p>
    </div>
</div>
@endsection